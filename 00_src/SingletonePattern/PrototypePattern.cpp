...
class Palette : public Singleton<Palette>
{
	friend class Singleton<Palette>;
public:
	//static Palette * g_pPallete;
	//static Palette* GetInstance()
	//{
	//	if (g_pPallete == nullptr)
	//	{
	//		g_pPallete = new Palette;
	//	}
	//	return g_pPallete;
	//}
private:
...
#define I_Palette Palette::GetInstance()

// Palette * Palette::g_pPallete = nullptr;

int main(void)
{
	Palette pt = I_Palette;
	I_Palette.Draw();
	GraphicEditor editer;
	editer.AddNewGraphics(I_Palette.GetSelectedObj());
