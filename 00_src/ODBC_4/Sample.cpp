...
void Select()
{
	...
	SQLCHAR UserID[10];
	SQLINTEGER sLen = sizeof(UserID);
	SQLRETURN sRet;
	SQLTCHAR Name[20] = { 0, };
	SQLTCHAR Pass[20] = { 0, };
	SQLLEN lName, lPass, lTm;
	...
	printf("찾을 ID입력: ");
	scanf_s("%s", UserID, 10);
	SQLBindParameter(g_hStmt, 1, SQL_PARAM_OUTPUT, SQL_C_SSHORT, SQL_SMALLINT, 0, 0, &sReturn, sizeof(sReturn), &cbRetParam);
	SQLBindParameter(g_hStmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_LONGVARCHAR, lBytes, 0, UserID, 0, NULL);
	sRet = SQLExecDirect(g_hStmt, (SQLTCHAR*)_T("{?=CALL usp_Select(?)}"), SQL_NTS);
	...
	if (sRet != SQL_SUCCESS && sRet != SQL_SUCCESS_WITH_INFO)
	{
		...
		SQLSMALLINT length;
		// 단순한 에러
		SQLError(g_hEnv, g_hDbc, g_hStmt, sqlstate, &sqlcode, buffer, SQL_MAX_MESSAGE_LENGTH + 1, &length);
		MessageBox(NULL, (LPTSTR)buffer, (LPTSTR)sqlstate, MB_OK);
...
void Selectinfo()
{
	...
	SQLTCHAR Pass[20] = { 0, };
	SQLLEN lName, lPass, lTm;
	SQLBindCol(g_hStmt, 2, SQL_C_CHAR, Name, sizeof(Name), &lName);
	SQLBindCol(g_hStmt, 3, SQL_C_CHAR, Pass, sizeof(Pass), &lPass);
	TIMESTAMP_STRUCT tm;
	SQLBindCol(g_hStmt, 4, SQL_C_TYPE_TIMESTAMP, &tm, sizeof(tm), &lTm);

	printf("찾을 ID입력: ");
	scanf_s("%s", UserID, 10);
	SQLBindParameter(g_hStmt, 1, SQL_PARAM_OUTPUT, SQL_C_SSHORT, SQL_SMALLINT, 0, 0, &sReturn, sizeof(sReturn), &cbRetParam);
	SQLBindParameter(g_hStmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_LONGVARCHAR, lBytes, 0, UserID, 0, NULL);
	sRet = SQLExecDirect(g_hStmt, (SQLTCHAR*)_T("{?=CALL usp_userinfo(?)}"), SQL_NTS);
	...
	//while ((sRet = SQLMoreResults(g_hStmt)) != SQL_NO_DATA_FOUND)
	//{
	//	printf("%d\n", sRet);
	//}
	SQLCloseCursor(g_hStmt);
